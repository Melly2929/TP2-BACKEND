...
const enterArgs = (argsByTerminal) => {
  const args = { 
    action: "",
    data: "",
  };
...
const executeActions= ({ action, data }) => {
  ...
  if (action.length === 0) {
    ...
    //  --tag seguido de una etiqueta, busca libros por etiqueta
    const booksByTag = books.getByTag(data);
    console.log(booksByTag);
  } else if (action === "--author" && data) {
    // --author seguido de un autor, busca libros por autor
    ...
    //  --name seguido de un nombre, busca libros por nombre
    ...
    // comando no reconocido, muestra un mensaje de error
...
const main = () => {
  ...
  const argsByTerminal = enterArgs(params);

  console.log(argsByTerminal);

  executeActions(argsByTerminal);
};
